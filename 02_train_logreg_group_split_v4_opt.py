#!/usr/bin/env python3
# 02_train_logreg_group_split_v2.py
# ------------------------------------------------------------
# Group-wise file split (C / D / ND): 2/3 train files, 1/3 test files (per group)
# Train a single Logistic Regression model, evaluate, and export TEST judgments
...
# Usage:
#   cd /path/to/out
#   python 02_train_logreg_group_split_v2.py
#
# Optional:
#   python 02_train_logreg_group_split_v2.py --seed 42 --threshold 0.5 --save_test_pred
# ------------------------------------------------------------
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    *_merged.csv の列構成に合わせて学習用特徴量を作る（被験者ごとに計算することが重要）
    ...
    # ------------------------------------------------------------
    # ✅ 追加（最適候補）: 時系列文脈（過去のみ）
    # - SpO2低下は遅れて出やすいので lag / 直近最大 を入れる
    # ------------------------------------------------------------
    df["spo2_drop_lag1"] = df["spo2_drop_from_baseline"].shift(1)
    df["spo2_drop_lag2"] = df["spo2_drop_from_baseline"].shift(2)
    df["hr_rise_lag1"]   = df["hr_rise_from_baseline"].shift(1)

    # 直近5分（過去のみ）の最大値：短時間のイベント蓄積を表現
    ROLL = 5
    df["spo2_drop_roll5_max"] = df["spo2_drop_from_baseline"].shift(1).rolling(ROLL, min_periods=ROLL).max()
    df["hr_rise_roll5_max"]   = df["hr_rise_from_baseline"].shift(1).rolling(ROLL, min_periods=ROLL).max()
    ...
    # 分内変動（optional列がある場合のみ）
    if ("spo2_min_pct" in df.columns) and ("spo2_max_pct" in df.columns):
        df["spo2_range_1m"] = df["spo2_max_pct"] - df["spo2_min_pct"]
    else:
        df["spo2_range_1m"] = np.nan

    if ("rr_min_ms" in df.columns) and ("rr_max_ms" in df.columns):
        # rr_min が最短RR(=最大HR), rr_max が最長RR(=最小HR)
        hr_max_1m = 60000.0 / df["rr_min_ms"]
        hr_min_1m = 60000.0 / df["rr_max_ms"]
        df["hr_range_1m"] = hr_max_1m - hr_min_1m
    else:
        df["hr_range_1m"] = np.nan
...
def main():
    ...
    for sid, df_raw in sorted(test_by_subject.items()):
        ...
        if args.save_test_pred or True:
