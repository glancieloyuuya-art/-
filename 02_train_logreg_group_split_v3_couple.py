...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    *_merged.csv の列構成に合わせて学習用特徴量を作る（被験者ごとに計算することが重要）
    ...
    # SpO2×HR の連動（相互作用）とフラグ特徴量（追加）
    # - couple_mul : SpO2低下量 × HR上昇量（連動の強さ）
    # - desat_flag : SpO2低下が閾値以上か（例: 3%低下）
    # - both_flag  : SpO2低下 AND HR上昇（典型パターン）
    SPO2_DROP_TH = 3.0  # (%)
    HR_RISE_TH   = 5.0  # (bpm)
    df["couple_mul"] = df["spo2_drop_from_baseline"] * df["hr_rise_from_baseline"]
    df["desat_flag"] = (df["spo2_drop_from_baseline"] >= SPO2_DROP_TH).astype(int)
    df["both_flag"]  = ((df["spo2_drop_from_baseline"] >= SPO2_DROP_TH) &
                        (df["hr_rise_from_baseline"]   >= HR_RISE_TH)).astype(int)
    # 1分差分
